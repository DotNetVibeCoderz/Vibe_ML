...
/// disk: <c>SCIENCEAPPGEN_APIKEY</c>, <c>SCIENCEAPPGEN_ENDPOINT</c>, <c>TAVILY_API_KEY</c>.
/// </para>
/// </remarks>
public sealed class ConfigurationService
{
    ...
    public AppSettings Load()
    {
        ...
        settings.ApiKey = Environment.GetEnvironmentVariable("SCIENCEAPPGEN_APIKEY")
                          ?? Get(values, "llm.apiKey", "");
        settings.Endpoint = Environment.GetEnvironmentVariable("SCIENCEAPPGEN_ENDPOINT")
        ...
        if (string.IsNullOrWhiteSpace(settings.ProjectsRoot))
        {
            settings.ProjectsRoot = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "ScienceAppGen");
        }
